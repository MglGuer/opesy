...
private:
    ...
    std::atomic<bool> processingInstruction{false}; // Flag to prevent multiple cores from processing same instruction
...
public:
    ...
    void stop() {
        ...
        std::cout << "Scheduler stopped." << std::endl << std::endl;
    }
    ...
    void coreWorker(int coreId) {
        while (running) {
            ...
            // Try to get work - either continue current process or get new one
            {
                std::lock_guard<std::mutex> lock(currentProcessMutex);
                
                if (currentProcess && currentProcess->canExecute() && !processingInstruction.exchange(true)) {
                    // We can continue with current process and we got the execution lock
                    processToExecute = currentProcess;
                    shouldExecute = true;
                } else if (!currentProcess && !processingInstruction.load()) {
                    // No current process, try to get one from queue
            ...
            if (shouldExecute && processToExecute) {
                // Execute one instruction
                processToExecute->executeInstruction(coreId);
                
                // Check if process is finished
                bool processFinished = processToExecute->hasFinished();
                
                // Release the instruction processing lock
                processingInstruction = false;
                
                if (processFinished) {
                    std::lock_guard<std::mutex> cpLock(currentProcessMutex);
                    if (currentProcess == processToExecute) {
                        currentProcess = nullptr;
                        
                        // Move to finished processes
                        std::lock_guard<std::mutex> pLock(processMutex);
                ...
                // Notify other cores that they can try to get work
                queueCV.notify_all();
            } else {
                // If we couldn't get work, wait a bit or wait for notification
                std::unique_lock<std::mutex> lock(queueMutex);
                queueCV.wait_for(lock, std::chrono::milliseconds(1), 
                    [this]() { return !processQueue.empty() || !running || (!processingInstruction.load() && currentProcess && currentProcess->canExecute()); });
...
// maybe copy paste FCFS then just modify to include the quantum stuff
};
...
void schedulerStop() {
    if (scheduler && scheduler->isRunning()) {
        ...
        std::cout << "Scheduler is not running." << std::endl << std::endl; //stopping scheduler
    }
}
