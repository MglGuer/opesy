...
public:
    ...
    void recordMemoryViolation(uint32_t address)
    {
        ...
        currentInstruction = instructions.size(); // Mark as finished
    }
...
void screen(std::string &screenCommand)
{
        ...
        while (true)
        {
            ...
            if (screenInput == "process-smi")
            {
                ...
                auto it = std::find_if(allProcesses.begin(), allProcesses.end(),
                                       ...
                                       { return p->getName() == newProcess->getName(); });
    ...
    else if (option == "-r" && !argument.empty())
    {
        ...
        auto finishedIt = std::find_if(finishedProcesses.begin(), finishedProcesses.end(),
                                       [&screenName](const Process *p)
                                       { return p->getName() == screenName; });

        if (finishedIt != finishedProcesses.end())
        {
            Process *finishedProcess = *finishedIt;

            // It checks specifically for the memory violation flag.
            ...
            {
                // This is the generic message for a normally finished process.
                std::cout << "Process \"" << screenName
                          << "\" has already finished execution and cannot be accessed."
                          << std::endl
                          << std::endl;
            }
            return; // Important: exits after handling the finished process.
        }
        ...
        for (auto &scr : screenList)
        {
            if (scr.screenName == screenName)
            {
                ...
                auto it = std::find_if(allProcesses.begin(), allProcesses.end(),
                                       [&screenName](const std::unique_ptr<Process> &p)
                                       { return p->getName() == screenName; });
                ...
                std::cout << "Time Created: " << curScreen.timeCreated << std::endl
                          << std::endl;
        ...
        if (found)
        {
            ...
            while (screenInput != "exit")
            {
                ...
                {
                    std::cout << "Invalid command. You can only input 'process-smi' or 'exit'. " << std::endl
                              << std::endl;
                }
            }
        }
